...
digitalspaghetti.password = {	
	'defaults' : {
		...
		'minChar': 6,
		'minCharText': '最小 %d 位字符',
		'colors': ["#f00", "#c06", "#f60", "#3c0", "#3f0"],
		'scores': [20, 30, 43, 50],
		'verdicts':	["很不安全", "不安全", "一般", "安全的", "非常安全"],
		'raisePower': 1.4,
		'debug': false
	},
	'ruleScores' : {
	...
	'attachWidget': function (element) {
		var output = ['<span id="password-strength">'];
	...
	'calculateScore': function (word) {
		...
		for (var key in digitalspaghetti.password.rules) if (digitalspaghetti.password.rules.hasOwnProperty(key)) {
			...
			jQuery('.password-strength-bar').css({'background-color': digitalspaghetti.password.strColor}).text(digitalspaghetti.password.strText);
		}
	}
};
