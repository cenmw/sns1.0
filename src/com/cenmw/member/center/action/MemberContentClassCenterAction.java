...
public class MemberContentClassCenterAction extends BaseAction {
	/**
	 * 会员文章分类 模块
	 */
	...
	public String delete() {
		if (ids != null && ids.length() > 0) {
			...
			for(int i=0;i<idarrs.length;i++){
				memberBlogClass = memberBlogClassCenterService.getMemberBlogClass(new Integer(idarrs[i].trim()));
	...
	public String save() {
		...
		if (memberBlogClass.getId() == null) {
			...
			memberBlogClass.setType(new Integer(2));
	...
	public String info() {
		memberInfo = (MemberInfo) session.get(ConstantUtils.MEMBERINFO);
		if (id > 0) {
			memberBlogClass = memberBlogClassCenterService.getMemberBlogClass(id);
			memberBlogClass.setMemberInfo(memberInfoCenterService.getMemberInfo(memberBlogClass.getMid()));
			String msg = (String) session.get("classmsg");
			if (msg != null && msg.equals("添加成功！")) {
				classlist = memberBlogClassCenterService.findMemberBlogClassInList(memberInfo.getId(),2);
			}
		}
		// 初始化信息
	 ...
	 */
	public String list() {
		String hql = " from MemberBlogClass where type=2 and isdel=0";
		Map map = new HashMap();
		String parameter = "";
		// 默认排序 最新时间
		String sortstr = "";
		...
		// 判断排序条件
		pageBean = memberBlogClassCenterService.findMemberBlogClassHQLList(hql, sortstr,
				map, currentPage, pageSize);
		pageBean.setAction("/membercenter/contentclasslist");
	...
	public List getnewlist(List pagelist) {
		...
		if (pagelist != null && pagelist.size() > 0) {
			for (int i = 0; i < pagelist.size(); i++) {
				MemberBlogClass mblog = (MemberBlogClass) pagelist.get(i);
				mblog.setMemberInfo(memberInfoCenterService.getMemberInfo(mblog.getMid()));
				newlist.add(mblog);
			}
		}
		return newlist;
