...
function change_init(v,o_i,o_ii,o_iii){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(s[i]).selectedIndex-1));
	};
	...
	with(ss){
		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				...
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
					//alert(ar[i]+"----"+options[i+1].value);
					if(ar[i]==o_i && o_i !='' && o_i !='null'){
						options[i+1].selected = true;
					}
				}//end for
			}
		}//end if v		
		if(++v<s.length){change_init(v,o_ii,o_iii);}
	}//End with
}
...
function _init_area(province,city,county){  //初始化函数
	...
	change_init(0,province,city,county);
}
