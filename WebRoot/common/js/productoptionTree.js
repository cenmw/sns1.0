...
(function($) {
	$.fn.optionTree = function(tree, options) {
		...
		// if true, once preselected items will be chosen, the preselect list is cleared. This is to allow
		...
		var default_lazy_load = function(value) {
			...
			if (options.loading_image !== '') {
				...
				if (checkname.indexOf("___") > 0||checkname.indexOf("____") > 0) {
					
					
					
					
					$("#searchbrand").append(
							$("<img>").attr('src', options.loading_image).attr(
									'class', 'optionTree-loader'));
					var _brandid = options.brandid;
					$.ajax( {
						type : 'GET',
						url : '/shop/center/getbrandselect',
						data : 'id=' + value + '&brandid=' + _brandid,
						success : function(data) {
							$("#searchbrand").empty();
							$('.optionTree-loader').remove();
							$("#searchbrand").append($(data));
						}
					});
					
					var relaturl='/shop/center/getproductrelatjson';
					...
					$.ajax( {
						...
						success : function(data) {
							...
							for(var i=0;i<json.length;i++){
								...
								if(name!='价格'){
									div.append("<span class=\"orelat\">"+name+":</span>");
									if(child){
										for(var j=0;j<child.length;j++){
											var cid=child[j].id;
											var cname=child[j].name;
											//
											var checkstr="";
											var coclass=options.coclass;
											if(coclass.length>0){
												coclass=coclass.substring(1,coclass.length-1);
												var coArr=coclass.split("-");
											...
											div.append("<span class=\"trelat\"><input class=\"relatval\" type=\"radio\" name=\"relat"+id+"\" value=\""+cid+"\" "+checkstr+"/>"+cname+"</span>");
										}
									}
								}else{
									...
									if(child){
										for(var j=0;j<child.length;j++){
											...
											div.append("<input type=\"radio\" style=\"display:none\" rel=\""+cnote+"\" name=\"relatprice\" value=\""+cid+"\" "+checkstr+"/>");
										}
									}
								}
					...
					if(checkname.indexOf("____") > 0){
						return;
					}
				} else {
					$("#searchbrand").empty();
					$("#relatbox").empty();
					$("<img>").attr('src', options.loading_image).attr('class',
							'optionTree-loader').insertAfter(input);

				}
			}

			$.getJSON(options.lazy_load, {
				id : value
			}, function(tree) {
				$('.optionTree-loader').remove();
				var prop;
			...
			// remove all dynamic options of lower levels
				...
				if (typeof tree === "object") { // many options exists for current nesting level
							...
							.change(
									function() {
										if (this.options[this.selectedIndex].value !== '') {
											if ($
													.isFunction(options.on_each_change)) {
												removeNested(name + '_');
												options.on_each_change
														.apply(
																this,
																[
					...
					if (jQuery.isFunction(options.choose)) {
						var level = $(this).siblings().andSelf().filter(
								'select').length;
					...
					if (text_to_choose !== '') {
						$("<option value=\"-1\">").html(text_to_choose).val('')
								.appendTo($select);
					}
					...
					$.each(tree, function(k, v) {
						...
						var o = $("<option>").html(label).attr('value', value);
