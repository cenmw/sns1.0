...
var curpage = 1;
var page = <s:property value="learnInfo.page" escape="false" />;
//显示上一页，下一页，保存按钮的控制
...
function nextpage(){
   if(page > 1){
      for(var i=1;i<=page;i++){
         document.getElementById("learnpage"+i).style.display="none";
      }
   }
   ++curpage;
   if(curpage<=page){
      document.getElementById("learnpage"+curpage).style.display=""; 
   }
   showlastnext();
}
//上一页
...
function checkcontentinfo(){
	var learntext=document.getElementsByName("learntext");
	var result = "";
	if(learntext.length>0){
	   for(i=0;i<learntext.length;i++){
	       var lti = learntext[i].value;
		   if(lti == ""){
		      lti = " ";
		   }
	       if(i>0){
	...
	document.getElementById("learnlogfrmid").submit();
}
</script>
</head>
<body>
...
<div class="content1 layout-control">
   ...
   <div class="content-right fl">
   		<h2 class="second-title"><a class="blue" href="<s:if test="backUrl == null">/</s:if><s:else><s:property value="backUrl"/></s:else>"><<返回上一页</a>学习中心</h2>
	
	  <div class="vedio-list">
        	<div class="pl-title">
            	<h1><s:property value="learnInfo.title" escape="false" /></h1>
                <p class="title-infor"><span><s:date name="learnInfo.ctime" format="yyyy-MM-dd"/></span><span>点击量：<s:property value="learnInfo.viewnumber" escape="false" /></span></p>
            </div>
			<form name="learnlogfrm" id="learnlogfrmid" method="post" action="/membercenter/learnlogsave">
			<input type="hidden" name="lid" value="<s:property value="learnInfo.id" escape="false" />" />
            ...
            <div class="pl-content" style="padding-left:30px; line-height:35px;"><s:property value="learnInfo.tcontent" escape="false" /></div>        
            <div class="clear"></div>
            </form> 
