...
import com.cenmw.comment.front.service.CommentIpFrontService;
...
public class CommentIpFrontAction extends BaseAction {
	/**
	 * 评论信息 模块
	 */
	private CommentIpFrontService commentIpFrontService; 
	...
	public String delete() {
		if (ids != null && ids.length() > 0) {
			...
			for(int i=0;i<idarrs.length;i++){
				commentIpFrontService.deleteCommentIpById(new Integer(idarrs[i].trim()));
			}
			String msg = "删除成功！";
			session.put("msg", msg);
		}
	...
	public String save() {
		...
		if (commentIp.getId() == null) {
			...
			commentIpFrontService.saveCommentIp(commentIp);
		}
		commentIpFrontService.updateCommentIp(commentIp);
		session.put("msg", msg);
		return SUCCESS;
	}
	...
	public String info() {
		if (id > 0) {
			commentIp = commentIpFrontService.getCommentIp(id);
		}
		// 初始化信息
		if (commentIp == null) {
	...
	public String list() {
		...
		// 判断排序条件
		pageBean = commentIpFrontService.findCommentIpHQLList(hql, sortstr,
				map, currentPage, pageSize);
		pageBean.setAction("/commentip/list");
	...
	public CommentIpFrontService getCommentIpFrontService() {
		return commentIpFrontService;
	}

	public void setCommentIpFrontService(
			CommentIpFrontService commentIpFrontService) {
		this.commentIpFrontService = commentIpFrontService;
	}
