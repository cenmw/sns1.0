	...
	$.fn.optionTree = function(tree, options) {
		...
		var default_lazy_load = function(value) {
			...
			if (options.loading_image !== '') {
				...
				var obj=$(this)[0];
				checkname=obj.getAttribute("name");
				if (checkname.indexOf("___") > 0) {
					...
					$.ajax( {
						type : 'GET',
						url : '/manager/product/getbrandselect',
				...
				}else if(checkname.indexOf("____") > 0){
					var _brandid = options.brandid;
					$("#searchbrand").empty();
					$("#searchbrand").append(
							$("<img>").attr('src', options.loading_image).attr(
					...
					$("<img>").attr('src', options.loading_image).attr('class',
							'optionTree-loader').insertAfter($(obj));
				}
		...
		return this.each(function() {
			var obj=$(this)[0];
			var name = obj.getAttribute("name") + "_";
