package com.cenmw.integral.front.action;
...
public class IntegralInfoFrontAction extends BaseAction {
	/**
	 * 积分信息 模块
	 */
	...
	public String delete() {
		if (ids != null && ids.length() > 0) {
			...
			for(int i=0;i<idarrs.length;i++){
				integralInfo = integralInfoFrontService.getIntegralInfo(new Integer(idarrs[i].trim()));
				integralInfo.setIsdel(new Integer(1));
				integralInfoFrontService.updateIntegralInfo(integralInfo);
			}
			String msg = "删除成功！";
			session.put("msg", msg);
		}
	...
	public String info() {
		if (id > 0) {
			integralInfo = integralInfoFrontService.getIntegralInfo(id);
		}
		// 初始化信息
	...
	public String list() {
		...
		// 判断排序条件
		pageBean = integralInfoFrontService.findIntegralInfoHQLList(hql, sortstr,
				map, currentPage, pageSize);
		pageBean.setAction("/integral/list");
	...
	public List getnewlist(List pagelist) {
		...
		if (pagelist != null && pagelist.size() > 0) {
			for (int i = 0; i < pagelist.size(); i++) {
				...
				iinfo.setSumscore(integralInfoFrontService.getIntegralByMid(iinfo.getMid()));
				newlist.add(iinfo);
			}
		}
		return newlist;
