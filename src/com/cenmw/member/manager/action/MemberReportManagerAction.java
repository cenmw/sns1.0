...
public class MemberReportManagerAction extends BaseAction {
	/**
	 * 会员举报 模块
	 */
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.DeleteMEMBERREPORT })
	public String delete() {
		if (ids != null && ids.length() > 0) {
			String[] idarrs = ids.split(",");
			for(int i=0;i<idarrs.length;i++){
				memberReport = memberReportManagerService.getMemberReport(new Integer(idarrs[i].trim()));
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.ADDMEMBERREPORT })
	public String save() {
		String msg = "修改成功！";
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.VIEWMEMBERREPORT })
	public String info() {
		if (id > 0) {
			memberReport = memberReportManagerService.getMemberReport(id);
			memberReport.setMemberInfo(memberInfoManagerService.getMemberInfo(memberReport.getMid()));
			memberReport.setRmemberInfo(memberInfoManagerService.getMemberInfo(memberReport.getRid()));
		}
		// 初始化信息
	...
	public String list() {
		...
		pageSize = 20;
		// 判断排序条件
		pageBean = memberReportManagerService.findMemberReportHQLList(hql, sortstr,
				map, currentPage, pageSize);
		pageBean.setAction("/manager/memberreport/list");
	...
	public List getnewlist(List pagelist) {
		...
		if (pagelist != null && pagelist.size() > 0) {
			for (int i = 0; i < pagelist.size(); i++) {
				...
				mreport.setMemberInfo(memberInfoManagerService.getMemberInfo(mreport.getMid()));
				mreport.setRmemberInfo(memberInfoManagerService.getMemberInfo(mreport.getRid()));
				newlist.add(mreport);
			}
		}
		return newlist;
