...
public class MemberPoliceManagerAction extends BaseAction {
	/**
	 * 警察局 模块
	 */
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.DeleteMEMBERPOLICE })
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.ADDMEMBERPOLICE })
	public String save() {
		String msg = "修改成功！";
	 ...
	 * @return
	 */
	@EmployeePermission(perm = { EmployeePermissionType.VIEWMEMBERPOLICE })
	public String info() {
		if (id > 0) {
	...
	@EmployeePermission(perm = { EmployeePermissionType.VIEWMEMBERPOLICE })
	public String list() {
		String hql = " from MemberReport where state=1 and isdel=0";
		Map map = new HashMap();
		String parameter = "";
		// 默认排序 最新时间
		String sortstr = "";
		...
		pageBean.setAction("/manager/memberpolice/list");
	...
	public List getnewlist(List pagelist) {
		...
		if (pagelist != null && pagelist.size() > 0) {
			for (int i = 0; i < pagelist.size(); i++) {
				...
				mpolice.setMemberInfo(memberInfoManagerService
						.getMemberInfo(mpolice.getMid()));
				mpolice.setRmemberInfo(memberInfoManagerService
